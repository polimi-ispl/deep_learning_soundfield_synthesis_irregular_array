...
def main():
    ...
    # Construct paths
    filter_dataset_path = '/nas/home/lcomanducci/soundfield_synthesis/dataset/linear_array/filters_config_nl_64_missing_'+str(number_missing_loudspeakers)+'.npy'
    ...
    epoch_to_plot = 100  # Plot evey epoch_to_plot epochs
    val_perc = 0.2
    ...
    """
    for n in range(d_.shape[0]):
        if np.max(np.abs(d_[n])) > 10:
            #plt.plot()
            print(str(n) +' max'+str(np.max(np.abs(d_[n]))))
            plt.plot(params_linear.src_pos_train[0,n],params_linear.src_pos_train[1,n],'k*')
    plt.show()
    """

    # P_gt = P_gt[:, params_linear.idx_cp] # --> should be coherent for circular array
    ...
    # Split train/val
    d_train, d_val, P_train, P_val, src_train, src_val = train_test_split(d_, P_gt, params_linear.src_pos_train.transpose(), test_size=val_perc)
    ...
    def train_step(d_, P_):
        ...
        metric_fn_train_real.update_state(train_utils.normalize_tensor(tf.math.abs(P_)),
                                          train_utils.normalize_tensor(tf.math.abs(p_est)))
        metric_fn_train_imag.update_state(train_utils.normalize_tensor(tf.math.angle(P_)),
                                          train_utils.normalize_tensor(tf.math.angle(p_est)))
        return loss_value_P
    ...
    for n_e in tqdm(range(epochs)):
        ...
        if not n_e % epoch_to_plot and plot_val:
            ...
            plt.subplot(311)
            sfs.plot2d.amplitude(p_pwd_real, params_linear.grid, xnorm=[0, 0, 0])
            sfs.plot2d.loudspeakers(params_linear.array.x, params_linear.array.n, selection)
            plt.title('pwd + F:' + str(params_linear.f_axis[idx_f]) + ' Hz')
            plt.subplot(312)
            sfs.plot2d.amplitude(p_gt, params_linear.grid, xnorm=[0, 0, 0])
            sfs.plot2d.loudspeakers(params_linear.array.x, params_linear.array.n, selection)
            plt.title('GT')

            plt.subplot(313)
            sfs.plot2d.amplitude(p_hat_real, params_linear.grid, xnorm=[0, 0, 0])
            sfs.plot2d.loudspeakers(params_linear.array.x, params_linear.array.n, selection)
            plt.title('est')
            #plt.show()
